...
void devteroflex_pack_archstate(DevteroflexArchState *devteroflex, CPUState *cpu ) {
    ...
    devteroflex->pc = QFLEX_RD_ARCH(pc)(cpu);
    devteroflex->asid = QFLEX_RD_ARCH(asid)(cpu);
    FLAGS_SET_NZCV(devteroflex->flags, QFLEX_RD_ARCH(nzcv)(cpu));
...
void devteroflex_unpack_archstate(CPUState *cpu, DevteroflexArchState *devteroflex) {
    ...
    assert(QFLEX_RD_ARCH(asid)(cpu) == devteroflex->asid);
...
bool devteroflex_compare_archstate(CPUState *cpu, DevteroflexArchState *devteroflex) {
    ...
    if(QFLEX_RD_ARCH(pc)(cpu) != devteroflex->pc) {
        qemu_log("Mismatched register PC is detected. \n QEMU: %lx, FPGA: %lx \n", QFLEX_RD_ARCH(pc)(cpu), devteroflex->pc);
        hasMismatch = true;
    }

    for(int i = 0; i < 32; ++i) {
        if(QFLEX_RD_ARCH(reg)(cpu, i) != devteroflex->xregs[i]) {
            qemu_log("Mismatched register %d is detected. \n QEMU: %lx, FPGA: %lx \n", i, QFLEX_RD_ARCH(reg)(cpu, i), devteroflex->xregs[i]);
            hasMismatch = true;
        }
    }
    
    if(QFLEX_RD_ARCH(nzcv)(cpu) != FLAGS_GET_NZCV(devteroflex->flags)) {
        qemu_log("Mismatched register NZCV is detected. \n QEMU: %x, FPGA: %lx \n", QFLEX_RD_ARCH(nzcv)(cpu), FLAGS_GET_NZCV(devteroflex->flags));
        hasMismatch = true;
    }

    if(QFLEX_RD_ARCH(asid)(cpu) != devteroflex->asid) {
        qemu_log("Mismatched register ASID is detected. \n QEMU: %lx, FPGA: %x \n", QFLEX_RD_ARCH(asid)(cpu), devteroflex->asid);
        hasMismatch = true;
    }
